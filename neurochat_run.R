...
library(NeuronChat)
### NeuronChat downloaded at Aug 3, 2023
...
#x = readRDS(file = paste0('NeuroChat_res/', label, '_neurochat.rds'))
comm_res = NULL
...
comm_res$pval = as.vector(pvalue)
db = do.call(rbind, x@DB)
comm_res_new = merge(comm_res, db, by.x = 'lr', by.y = 0)
comm_res_new = subset(comm_res_new, strength>0 & pval < 0.05)
saveRDS(x, file = paste0('NeuroChat_res/', label, '_neurochat.rds'))
write.table(as.matrix(comm_res_new), file = paste0('NeuroChat_res/', label, '_neurochat.tsv'), sep = '\t', quote = F, row.names = F)